    ...
    for k in bots_data():
        ...
        if r:
            ...
            if hashlib.sha1("&".join(p).encode()).hexdigest() == d.get("sha1_hash"):
                ...
                cnx.commit()
                try:
                    inv = await b[k].create_chat_invite_link(chat_id=cfg["CH"], name=f"x{r[0]}")
                    await b[k].send_message(r[0], f"Оплата прошла! Канал: {inv.invite_link}")
                    lg.info(f"{k}: Pay {i} OK")
                except:
                    await b[k].send_message(r[0], "Проблема с каналом. Пишите @Support")
                    lg.error(f"{k}: Invite fail {i}")
            cr.close()
            db.putconn(cnx)
