def get_bots():
    c = {
        "TON": "UQBLNUOpN5B0q_M2xukAB5MsfSCUsdE6BkXHO6ndogQDi5_6",
        "BTC": "bc1q5xq9m473r8nnkx799ztcrwfqs0555fs3ulw9vr",
        "USDT": "TQzs3V6QHdXb3CtNPYK9iMWuvvrYCPt6vE"
    }
    bots = {
        "x1": {"T": "7669060547:AAF1zdVIBcmmFKQGhQ7UGUT8foFKW4EBVxs", "Y": "4100118178122985", "S": "CoqQlgE3E5cTzyAKY1LSiLU1", "CH": -1002640947060, "P": 600.00, "C": c},
        "x2": {"T": "8173622705:AAE88BPX5k1mHuwFFBlWJS8ixxa36EmuCC0", "Y": "4100118178122985", "S": "CoqQlgE3E5cTzyAKY1LSiLU1", "CH": -1002609563244, "P": 3.00, "C": c}
    }
    # Добавляем ещё 18 ботов с фиктивными данными
    for i in range(3, 21):
        bots[f"x{i}"] = {
            "T": f"123456789{i}:AAFakeToken{i}ForBotX{i}",  # Замени реальным токеном
            "Y": "4100118178122985",  # Замени, если нужен другой кошелёк
            "S": "CoqQlgE3E5cTzyAKY1LSiLU1",  # Замени, если другой секрет
            "CH": -1002000000000 - i,  # Замени реальным ID канала
            "P": 500.00 + i * 10,  # Пример цены, замени
            "C": c
        }
    return bots
